...
mt753x_trap_frames(struct mt7530_priv *priv)
{
	...
	regmap_update_bits(priv->regmap, MT753X_BPC,
			   PAE_BPDU_FR | PAE_EG_TAG_MASK | PAE_PORT_FW_MASK | BPDU_EG_TAG_MASK | BPDU_PORT_FW_MASK,
			   PAE_BPDU_FR | PAE_EG_TAG(MT7530_VLAN_EG_UNTAGGED) | PAE_PORT_FW(TO_CPU_FW_CPU_ONLY) | BPDU_EG_TAG(MT7530_VLAN_EG_UNTAGGED) | TO_CPU_FW_CPU_ONLY);

	/* Trap frames with :01 and :02 MAC DAs to the CPU port(s) and egress
	 * them VLAN-untagged.
	 */
	regmap_update_bits(priv->regmap, MT753X_RGAC1,
			   R02_BPDU_FR | R02_EG_TAG_MASK | R02_PORT_FW_MASK | R01_BPDU_FR | R01_EG_TAG_MASK | R01_PORT_FW_MASK,
			   R02_BPDU_FR | R02_EG_TAG(MT7530_VLAN_EG_UNTAGGED) | R02_PORT_FW(TO_CPU_FW_CPU_ONLY) | R01_BPDU_FR | R01_EG_TAG(MT7530_VLAN_EG_UNTAGGED) | TO_CPU_FW_CPU_ONLY);

	/* Trap frames with :03 and :0E MAC DAs to the CPU port(s) and egress
	 * them VLAN-untagged.
	 */
	regmap_update_bits(priv->regmap, MT753X_RGAC2,
			   R0E_BPDU_FR | R0E_EG_TAG_MASK | R0E_PORT_FW_MASK | R03_BPDU_FR | R03_EG_TAG_MASK | R03_PORT_FW_MASK,
			   R0E_BPDU_FR | R0E_EG_TAG(MT7530_VLAN_EG_UNTAGGED) | R0E_PORT_FW(TO_CPU_FW_CPU_ONLY) | R03_BPDU_FR | R03_EG_TAG(MT7530_VLAN_EG_UNTAGGED) | TO_CPU_FW_CPU_ONLY);
}
...
mt7530_port_set_vlan_unaware(struct dsa_switch *ds, int port)
{
	...
	regmap_update_bits(priv->regmap, MT7530_PVC_P(port),
			   ...
			   VLAN_ATTR(MT7530_VLAN_TRANSPARENT) | PVC_EG_TAG(MT7530_VLAN_EG_CONSISTENT) | MT7530_VLAN_ACC_ALL);
...
mt7530_port_set_vlan_aware(struct dsa_switch *ds, int port)
{
	...
	if (dsa_is_user_port(ds, port)) {
		...
		regmap_update_bits(priv->regmap, MT7530_PVC_P(port),
				   ...
				   VLAN_ATTR(MT7530_VLAN_USER) | PVC_EG_TAG(MT7530_VLAN_EG_DISABLED));
	} else {
...
static int mt7988_setup(struct dsa_switch *ds)
{
	...
	if (priv->id == ID_AN7583)
		regmap_update_bits(priv->regmap, AN7583_GEPHY_CONN_CFG,
				   AN7583_CSR_DPHY_CKIN_SEL | AN7583_CSR_PHY_CORE_REG_CLK_SEL | AN7583_CSR_ETHER_AFE_PWD,
				   AN7583_CSR_DPHY_CKIN_SEL | AN7583_CSR_PHY_CORE_REG_CLK_SEL | FIELD_PREP(AN7583_CSR_ETHER_AFE_PWD, 0));
