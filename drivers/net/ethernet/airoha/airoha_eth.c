...
static int airoha_setup_phylink(struct net_device *dev)
{
	...
	count = fwnode_phylink_pcs_parse(dev_fwnode(&dev->dev), NULL);
	if (count < 0)
		return count;

	available_pcs = kcalloc(count, sizeof(*available_pcs), GFP_KERNEL);
	if (!available_pcs)
		return -ENOMEM;
	...
	port->phylink_config.num_available_pcs = count;
