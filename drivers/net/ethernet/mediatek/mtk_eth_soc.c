				 ...
				 bool tx_clk_stop)
{
	...
	if (mtk_is_netsys_v3_or_greater(mac->hw) && mtk_interface_mode_is_xgmii(mac->interface))
		return -EOPNOTSUPP;
...
static int mtk_add_mac(struct mtk_eth *eth, struct device_node *np)
{
	...
	if (MTK_HAS_CAPS(mac->hw->soc->caps, MTK_SGMII)) {
		...
		if (mtk_is_netsys_v3_or_greater(mac->hw)) {
			...
			count = fwnode_phylink_pcs_parse(of_fwnode_handle(np), NULL);
			if (count < 0)
				err = count;
			...
			err = fwnode_phylink_pcs_parse(of_fwnode_handle(np), mac->available_pcs);
			if (err)
				goto free_netdev;
