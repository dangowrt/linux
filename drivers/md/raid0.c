...
static int create_strip_zones(mddev_t *mddev)
{
	...
	list_for_each_entry(rdev1, &mddev->disks, same_set) {
		...
		list_for_each_entry(rdev2, &mddev->disks, same_set) {
			...
			// -> [J.Chiang], 2012/04/24 - Added to solve raid0 accessing failed when Ethernet accelerator
			//	is enabled and HDD capacities is not same. To use smaller HDD as rdev's sectors count.
			else
			{
				if ( rdev2->sectors < rdev1->sectors )
					rdev1->sectors = rdev2->sectors;
				else
					rdev2->sectors = rdev1->sectors;
	
				printk(KERN_INFO "raid0:   FORCE EQUAL\n");
				c = 1;
				break;
			}
			// <- End.
