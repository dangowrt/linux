		...
		union xhci_trb *trb, struct xhci_event_cmd *event)
{
	...
	list_for_each(entry, &ep->cancelled_td_list) {
		...
		xhci_dbg(xhci, "Cancelling TD starting at %p, 0x%llx (dma).\n",
				cur_td->first_trb,
				(unsigned long long)etxhci_trb_virt_to_dma(cur_td->start_seg, cur_td->first_trb));
		...
		int slot_id)
{
	...
	for (i = 0; i < config->desc.bNumInterfaces; i++) {
		...
		if (intf->desc.bInterfaceProtocol == 0x50)
			break;
	}

	if (i == config->desc.bNumInterfaces)
		return;
		...
		union xhci_trb *event)
{
	...
	if ((temp & PORT_PLC) && (temp & PORT_PLS_MASK) == XDEV_RESUME) {
		...
		if (DEV_SUPERSPEED(temp)) {
			...
			xhci_writel(xhci, temp1, addr);
			slot_id = etxhci_find_slot_id_by_port(xhci, port_id);
			if (!slot_id) {
	...
	if (temp & PORT_WRC) {
		temp1 = etxhci_port_state_to_neutral(temp);
		temp1 |= (PORT_WRC | PORT_RC);
		mask = PORT_CSC | PORT_PE | PORT_CONNECT;
		if ((temp & mask) == mask)
			temp1 |= PORT_CSC;
		...
		if ((temp & mask) == mask) {
			...
			xhci->port_c_connection |= 1 << (port_id - 1);
		}
	}
	...
	struct xhci_virt_ep *ep, int *status)
{
	...
	etxhci_debug_trb(xhci, xhci->event_ring->dequeue);
		...
		struct urb *urb, int slot_id, unsigned int ep_index)
{
	...
	return etxhci_queue_bulk_tx(xhci, GFP_ATOMIC, urb, slot_id, ep_index);
}
...
static int xhci_prev_queue_ctrl_tx(struct xhci_hcd *xhci,
	struct urb *urb, int slot_id)
{
	struct usb_ctrlrequest *setup;
	struct xhci_ep_ctx *ep_ctx;
	unsigned int ep_addr, ep_index;

	setup = (struct usb_ctrlrequest *) urb->setup_packet;
	ep_addr = le16_to_cpu(setup->wIndex) & 0xff;
	ep_index = ((ep_addr & USB_ENDPOINT_NUMBER_MASK) * 2) +
		((ep_addr & USB_ENDPOINT_DIR_MASK) ? 1 : 0) - 1;
	ep_ctx = etxhci_get_ep_ctx(xhci, xhci->devs[slot_id]->out_ctx, ep_index);
	if (setup->bRequest != USB_REQ_CLEAR_FEATURE)
		return 0;
	if (setup->wValue != cpu_to_le16(USB_ENDPOINT_HALT))
		return 0;
	if ((ep_ctx->ep_info & cpu_to_le32(EP_STATE_MASK)) ==
		cpu_to_le32(EP_STATE_HALTED))
		return 0;
		...
		struct urb *urb, int slot_id, unsigned int ep_index)
{
	...
	ret = xhci_prev_queue_ctrl_tx(xhci, urb, slot_id);
	if (ret)
		return ret;
