...
int etxhci_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	etxhci_debug_ring(xhci, ep_ring);
		...
		struct xhci_slot_ctx *slot_ctx)
{
	...
	if (slot_ctx->dev_info & cpu_to_le32(DEV_HUB) ||
		USB_CLASS_HUB != udev->descriptor.bDeviceClass ||
		udev->descriptor.bDeviceProtocol == 3)
		return;
	...
	for (i = 0; i < 3; i++) {
		ret = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0),
			...
			USB_DT_HUB << 8, 0, descriptor, sizeof(*descriptor),
	...
	if (ret < 0)
		goto fail;
	else if (descriptor->bNbrPorts > USB_MAXCHILDREN)
		goto fail;
...
int etxhci_check_bandwidth(struct usb_hcd *hcd, struct usb_device *udev)
{
	...
	xhci_get_hub_device_info(xhci, udev, slot_ctx);
