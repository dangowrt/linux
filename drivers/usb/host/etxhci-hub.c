		...
		u16 wIndex, __le32 __iomem *addr, u32 port_status)
{
	...
	case USB_PORT_FEAT_C_CONNECTION:
		if (xhci->port_c_connection & 1 << wIndex) {
			xhci->port_c_connection &= ~(1 << wIndex);
			return;
		}
		status = PORT_CSC;
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetPortStatus:
		...
		if ((temp & PORT_PLS_MASK) == XDEV_COMPLIANCE
			&& !(temp & PORT_RESET)
			&& (temp & PORT_POWER)) {
			temp1 = etxhci_port_state_to_neutral(temp);
			temp1 |= PORT_WR;
...
int etxhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	for (i = 0; i < ports; i++) {
		...
		temp = xhci_readl(xhci, addr);
		if ((temp & mask) != 0 ||
			(temp & PORT_PLS_MASK) == XDEV_COMPLIANCE ||
			(xhci->port_c_connection & 1 << i)	||
