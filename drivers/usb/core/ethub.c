...
// -> [Walker Chen], 2010/01/28 - USB3.0 find device
extern unsigned int ethusb_device_in_use;
// <- End.
	...
	if (ret < 4) {
		...
		*change = le16_to_cpu(hub->status->port.wPortChange);
		...
		if ((hub->hdev->parent != NULL) &&
			...
			if ((*status & USB_SS_PORT_STAT_SPEED) == USB_PORT_STAT_SPEED_5GBPS)
...
static unsigned get_port_link_stat(struct usb_hub *hub, unsigned portstatus)
{
	unsigned pls;
	...
	if ((hub->hdev->parent != NULL) &&
			hub_is_superspeed(hub->hdev)) {
		pls = (portstatus & 0x00c0) >> 1;
		pls |= (portstatus & 0x6000) >> 6;
	}
 ...
 init2:
	...
	for (port1 = 1; port1 <= hdev->maxchild; ++port1) {
		...
		if (!udev || udev->state == USB_STATE_NOTATTACHED) {
			...
			if (udev || (portstatus & USB_PORT_STAT_CONNECTION))
				set_bit(port1, hub->change_bits);
			if (get_port_link_stat(hub, portstatus)
...
void ethub_usb_disconnect(struct usb_device **pdev)
{
	...
	// -> [Walker Chen], 2010/01/28 - USB3.0 find device
	//printk("USB3.0:ethhub:usb_disconnect\n" );
	ethusb_device_in_use--;
	//printk("ethusb_device_in_use=%d\n",ethusb_device_in_use);
	// <- End.
...
static int usb_configure_device(struct usb_device *udev)
{
	...
	// -> [Walker Chen], 2010/01/28 - USB3.0 find device
	//if(err==0)printk("USB3.0:ethHUB:usb_configure_device\n");
	ethusb_device_in_use++;
	//printk("ethusb_device_in_use=%d\n",ethusb_device_in_use);
	// <- End.
fail:
...
int usb_is_etron_hcd(struct usb_device *udev)
{
	...
	if (hcd->driver &&
		!strcmp(hcd->driver->description, "etxhci_hcd"))
		return 1;
