	...
	loff_t		 length)
{
	...
	retval = xfs_ioc_space(ip, inode, filp, ioflags, XFS_IOC_RESVSP64, &info);
	if (likely(!retval)) {
		int extents = inode->i_op->get_extents(inode);

		// Limit the number of extents to avoid memory allocation problems with 
		// large filemaps
		if (extents > FAST_NUM_EXTENTS_LIMIT) {
//			printk(KERN_INFO "xfs_preallocate() Inode %p too many extents (%d > %d)\n",
//				inode, extents, FAST_NUM_EXTENTS_LIMIT);
			retval = -EFBIG;
		}
	}
...
//printk("xfs_reset_preallocate() inode %p file %p, start %lld, length %lld\n", inode, filp, start, length);
